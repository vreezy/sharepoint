...
export function createUrl(settings: IListSettings): string {
  const spWeb = ""; // there are many ways to get the sp web url -> context.pageContext.web.absoluteUrl or  window.location.???
  ...
  if (settings.customQuery) {
    ...
    let orderByQuery = '';
    let selectQuery = '';
    let top = settings.top ? '&$top=' + settings.top : '&$top=10000';
